...
class NameDialog(QDialog):
    ...
    def __init__(self):
        ...
        self.grid_layout.addWidget(QLabel('<h2 style="font-family: sans-serif">'
                                          '<center>Welcome to the Smart Voice Assistant <center/><h2/>'))
        self.grid_layout.addWidget(
            QLabel('<center>Please Enter Your Name <center/>'))
...
class Splash(QSplashScreen):
    ...
    def __init__(self):
        super().__init__(QPixmap('images/Wavy_Tech-07_Single-03.jpg'))
        self.timer = QTimer(app)
        self.timer.setSingleShot(True)
        self.showMessage(
            f'<center style="font-family: sans-serif;"><h2>WELCOME {name} TO YOUR SMART VOICE ASSISTANT</h2><center/>',
    ...
    def initialize(self):
        ...
        def welcome_text():
            self.clearMessage()
            self.setPixmap(QPixmap('images/5209989.jpg'))
            self.repaint()
            message = f'<h2>WELCOME {name} TO YOUR SMART VOICE ASSISTANT.</h2>\n'
            message += 'FOR SWITCHING TO VOICE, INTERNET CONNECTION IS NEEDED'
        ...
        def welcome_voice():
            if voice.synthesize_speech(f'Welcome {name} to your Smart voice assistant'):
                voice.speak()
                self.close()
            else:
                welcome_text()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.pushAbout.clicked.connect(lambda: self.open_url("https://www.linkedin.com/in/abhishek-kumar-9a5106185/"))
    ...
    def append_chatbot_history(self, string):
        self.ui.history.append(
            "<h3>Voice Assistant said  : {}</h3>  ".format(string))
